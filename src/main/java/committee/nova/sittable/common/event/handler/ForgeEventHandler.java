...
public class ForgeEventHandler {
    ...
    public void onInteract(PlayerInteractEvent.RightClickBlock e) {
        e.setCanceled(Utilities.trySit(e.getWorld(), e.getPos(), e.getFace(), e.getEntityPlayer()));
    }

    @SubscribeEvent
    public void onSittableRegister(SittableRegisterEvent e) {
        if (Utilities.isInDevEnv()) e.registerSittable(new SittableRegistry(Blocks.WOODEN_SLAB, (s, p, f) -> {
